...
def current_finishes_query() -> str:
    return """SELECT SUM(CASE WHEN place = 1 THEN 1 ELSE 0 END) as first,
        ...
        FROM (
            SELECT tr.tournament_id, tr.angler1_id, tr.angler2_id,
                DENSE_RANK() OVER (PARTITION BY tr.tournament_id ORDER BY tr.total_weight DESC) as place
            FROM team_results tr
            JOIN tournaments t ON tr.tournament_id = t.id
            JOIN events e ON t.event_id = e.id
            WHERE e.year = :current_year
        ) AS ranked
        WHERE ranked.angler1_id = :user_id OR ranked.angler2_id = :user_id"""
