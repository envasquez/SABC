"""Vote validation helpers for poll voting."""
...
def validate_poll_state(poll_id: int, user_id: int) -> Optional[str]:
    """Validate poll state and return error message if invalid, None if valid."""
    poll_check = db(
...
def validate_tournament_location_vote(vote_data: dict) -> Tuple[Optional[str], Optional[str]]:
    """Validate tournament location vote data. Returns (option_text, error_message)."""
...
def get_or_create_option_id(poll_id: int, option_text: str, vote_data: dict) -> Optional[int]:
    """Get existing option ID or create new option, return option ID."""
    existing_option = db(
