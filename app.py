...
def time_format_filter(time_str):
    """Format time string from 24-hour to AM/PM format."""
    if not time_str:
        return ""
    try:
...
def month_number_filter(date_str):
    """Extract month number from date string (zero-padded)."""
    if not date_str:
        return "00"
    try:
        # Parse the date string (assuming YYYY-MM-DD format from database)
...
def load_lakes_data():
    """Load lakes and ramps data from YAML file."""
    global _lakes_data_cache
    if _lakes_data_cache is None:
        lakes_file = os.path.join(os.path.dirname(__file__), "data", "lakes.yaml")
        try:
            with open(lakes_file, "r") as f:
                _lakes_data_cache = yaml.safe_load(f)
        except FileNotFoundError:
            print(f"Warning: {lakes_file} not found, using empty data")
            _lakes_data_cache = {}
    return _lakes_data_cache


def get_lakes_list():
    """Get lakes as list of (id, name, location) tuples for compatibility with database format."""
    ...
    for lake_id, (lake_key, lake_info) in enumerate(lakes_data.items(), 1):
        ...
        # Extract location from display_name or use a default
        location = "Central Texas"  # default location
        lakes.append((lake_id, display_name, location))
    return lakes


def get_ramps_for_lake(lake_id):
    """Get ramps for a specific lake ID in tuple format (id, name, lake_id)."""
    # Convert lake_id to lake_key first
    ...
    if lake_key in lakes_data and "ramps" in lakes_data[lake_key]:
        ...
        for ramp_idx, ramp in enumerate(lakes_data[lake_key]["ramps"]):
            ramp_id = f"{lake_key}_{ramp_idx}"  # Create unique ramp ID
...
def get_all_ramps():
    """Get all ramps with lake association for compatibility."""
    lakes_data = load_lakes_data()
    ramps = []
    ramp_id = 1
    for lake_id, (lake_key, lake_info) in enumerate(lakes_data.items(), 1):
...
def find_ramp_name_by_id(ramp_id):
    """Get ramp name by ID."""
    # Ramp IDs are in format "lake_key_index"
    if "_" in str(ramp_id):
        ...
        if len(parts) == 2:
            ...
            try:
                ...
                # Find the numeric lake_id from the lake_key
                lakes = get_lakes_list()
                lake_id = None
                for l_id, l_name, l_location in lakes:
                    # Check if this lake matches the lake_key
                ...
                if lake_id is not None:
                    ...
                    if 0 <= ramp_index < len(ramps_tuples):
                        return ramps_tuples[ramp_index][
                            1
                        ]  # Return name from tuple (id, name, lake_id)
...
def validate_lake_ramp_combo(lake_id, ramp_id):
    ...
    # Get ramps for this lake using the numeric lake_id
    ramps_tuples = get_ramps_for_lake(lake_id)
    if not ramps_tuples:
        return False

    # Check if the ramp_id exists in the ramps for this lake
    # ramps_tuples format: [(id, name, lake_id), ...]
    for ramp_tuple in ramps_tuples:
        ramp_id_from_tuple = ramp_tuple[0]  # First element is the ramp ID
...
def find_lake_data_by_db_name(db_lake_name):
    """Map database lake name to YAML lake data and display name."""
    if not db_lake_name:
    ...
    # Create a mapping of common variations to YAML keys
    name_mappings = {
        "belton": "belton",
        "travis": "travis",
    ...
    # Fallback: try to find by partial match in YAML keys or display names
    for key, info in lakes_data.items():
...
async def admin_page(request: Request, page: str, upcoming_page: int = 1, past_page: int = 1):
    ...
    if page == "events":
        ...
        ctx["users"] = db(
            "SELECT id, name, email, member, is_admin, active FROM anglers ORDER BY active DESC, is_admin DESC, member DESC, name"
        )
...
async def admin_news(request: Request):
    ...
    news_items = db("""
        SELECT n.id, n.title, n.content, n.created_at, n.published, n.priority,
               n.expires_at, a.name as author_name
        FROM news n
...
async def update_user(
    ...
    """Update user details with comprehensive validation."""
    if not (user := u(request)) or not user.get("is_admin"):
        return RedirectResponse("/login")
    ...
    try:
        ...
        elif not member:  # Guest without email - generate default
            # Create first.last@sabc.com format
            name_parts = name.strip().lower().split()
            if len(name_parts) >= 2:
                first_name = name_parts[0]
                last_name = name_parts[-1]
                # Remove special characters for email
                first_clean = "".join(c for c in first_name if c.isalnum())
                last_clean = "".join(c for c in last_name if c.isalnum())
        ...
        db(
            """
            UPDATE anglers
            SET name = :name, email = :email, member = :member,
        ...
        # Make constraint violations clearer
        if "UNIQUE constraint failed: anglers.email" in error_msg:
            # Find who has this email
            existing = db(
                "SELECT name FROM anglers WHERE email = :email AND id != :id",
                {"email": update_params["email"], "id": user_id},
            )
            if existing:
                error_msg = f"Email '{update_params['email']}' already belongs to {existing[0][0]}"
            else:
                error_msg = f"Email '{update_params['email']}' is already in use"
        ...
        return JSONResponse(
            {
                ...
                "name": user_data[1],
                "email": user_data[2],
                "member": bool(user_data[3]),
                "is_admin": bool(user_data[4]),
                "active": bool(user_data[5]),
            }
        )
...
async def home_paginated(request: Request, page: int = 1):
    user = u(request)
    items_per_page = 6  # Show 6 tournament cards per page
    ...
    for t in all_tournaments_query:
        ...
        # Add top 3 TEAM finishers for completed tournaments using REAL team results
    ...
    # Get latest news from news table
    latest_news = db("""
        SELECT id, title, content, created_at, 'news' as type
        FROM news
        WHERE published = 1
