...
class TournamentScraper:
    ...
    def __init__(self, base_url: str = REFERENCE_SITE):
        ...
        self.session.headers.update({
            'User-Agent': 'Mozilla/5.0 (SABC Tournament Importer)'
        })
            ...
            if date_match:
                ...
                try:
                    ...
                    try:
                        ...
                        month_match = re.search(r'(January|February|March|April|May|June|July|August|September|October|November|December)', title)
                        if month_match:
                        ...
                        try:
                            ...
                            for cell in cells[2:]:
                                try:
                                    ...
                                    if weight_text and not weight_text.isdigit():  # Avoid fish count
                ...
                for row in table.find_all('tr')[1:]:  # Skip header row
                    ...
                    if len(cells) >= 3:
                        try:
                            first_name = cells[1].get_text(strip=True)
                            last_name = cells[2].get_text(strip=True)
                            points_str = cells[3].get_text(strip=True) if len(cells) >= 4 else "0"
    ...
    def _mark_buy_in_anglers(self, results: List[TournamentResult], buy_in_anglers: Dict[str, Tuple[int, int]]) -> None:
    ...
    def _add_buy_in_only_anglers(self, results: List[TournamentResult], buy_in_anglers: Dict[str, Tuple[int, int]]) -> None:
...
class DatabaseImporter:
    ...
    def _import_single_tournament(self, conn, tournament: Tournament) -> None:
        ...
        lake_id = self._ensure_lake_exists(conn, tournament.lake_name, tournament.google_maps_iframe)
        ramp_id = self._ensure_ramp_exists(conn, lake_id, tournament.ramp_name, tournament.google_maps_iframe)
    ...
    def _create_event(self, conn, tournament: Tournament) -> int:
        ...
        if tournament.start_time:
            try:
                ...
                if 'am' in time_str.lower() or 'pm' in time_str.lower():
                    ...
                    time_str = time_str.lower().replace('am', ' am').replace('pm', ' pm').replace('  ', ' ').strip()
        ...
        existing = conn.execute(
            text("SELECT id FROM events WHERE date = :date"),
    ...
    def _ensure_lake_exists(self, conn, lake_name: str, google_maps_iframe: Optional[str]) -> int:
        ...
        if existing:
            ...
            if google_maps_iframe:
                conn.execute(
                    text("UPDATE lakes SET google_maps_iframe = :iframe WHERE id = :id AND google_maps_iframe IS NULL"),
    ...
    def _ensure_ramp_exists(self, conn, lake_id: int, ramp_name: Optional[str], google_maps_iframe: Optional[str]) -> int:
        ...
        if existing:
            ...
            if google_maps_iframe:
                conn.execute(
                    text("UPDATE ramps SET google_maps_iframe = :iframe WHERE id = :id AND google_maps_iframe IS NULL"),
    ...
    def _create_tournament(self, conn, tournament: Tournament, event_id: int, lake_id: int, ramp_id: int) -> int:
    ...
    def _ensure_angler_exists(self, conn, angler_name: str) -> int:
        ...
        existing = conn.execute(
            text("SELECT id FROM anglers WHERE name = :name"),
            {"name": angler_name}
        ).fetchone()
    ...
    def _create_result(self, conn, tournament_id: int, angler_id: int, result: TournamentResult) -> None:
        ...
        existing = conn.execute(
            text("SELECT id FROM results WHERE tournament_id = :tournament_id AND angler_id = :angler_id"),
    ...
    def _create_team_result(self, conn, tournament_id: int, angler1_id: int, angler2_id: int, team_result: TeamResult) -> None:
...
The script follows the SABC database schema and type safety requirements.
    ...
    parser.add_argument('year', type=int, help='Year to import tournaments for (e.g., 2022, 2023, 2024)')
