...
def main():
    ...
    lake_mappings = {
        "INKS LAKE": "INKS LAKE",  # Keep the uppercase one as it's cleaner
        "inks": "inks_lake",  # Actually merge the other way - remove the lowercase one
    ...
    with engine.begin() as conn:
        for merge in merges:
            ...
            print(f"Merging '{remove_lake.display_name}' (ID: {remove_lake.id}) into '{keep_lake.display_name}' (ID: {keep_lake.id})")
            ...
            conn.execute(
                text("DELETE FROM lakes WHERE id = :remove_id"),
                {"remove_id": remove_lake.id}
            )
