...
if first_block_start > 0:
    ...
    next_section = content.find('{% endif %} <!-- Close else for user_has_voted (admin tab) -->', search_start)
    if next_section > 0:
...
second_block_start = content.find('<!-- Seasonal Tournament History for upcoming tournament polls -->')
if second_block_start > 0:
    search_start = second_block_start
    # Find the next major section
    next_section = content.find('{% endif %} <!-- Close else for user_has_voted (upcoming tab) -->', search_start)
    if next_section > 0:
        temp = content[search_start:next_section]
        endif_pos = temp.rfind('{% endif %}')
        if endif_pos > 0:
...
third_block_start = content.find('<!-- Seasonal Tournament History -->', second_block_start if second_block_start > 0 else 0)
