...
class Angler(Base):
    ...
    email: Mapped[Optional[str]] = mapped_column(Text, index=True)  # Index for login performance
...
class PollVote(Base):
    ...
    cast_by_admin_id: Mapped[Optional[int]] = mapped_column(
        Integer, ForeignKey("anglers.id", ondelete="SET NULL"), nullable=True, index=True
    )
...
class Tournament(Base):
    ...
    name: Mapped[str] = mapped_column(Text, nullable=False)
    lake_id: Mapped[Optional[int]] = mapped_column(
        Integer, ForeignKey("lakes.id", ondelete="SET NULL"), index=True
    )
    ramp_id: Mapped[Optional[int]] = mapped_column(
        Integer, ForeignKey("ramps.id", ondelete="SET NULL"), index=True
    )
    lake_name: Mapped[Optional[str]] = mapped_column(Text)
    ramp_name: Mapped[Optional[str]] = mapped_column(Text)
    start_time: Mapped[Optional[time]] = mapped_column(Time)
