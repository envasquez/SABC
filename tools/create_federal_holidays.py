...
def calculate_federal_holidays(year: int) -> List[Tuple[str, date, str]]:
    ...
    holidays.append(("New Year's Day", date(year, 1, 1), "Federal holiday celebrating the beginning of the new year."))
    
    # Martin Luther King Jr. Day - 3rd Monday in January
    mlk_day = get_nth_weekday(year, 1, 0, 3)  # 0 = Monday
    holidays.append(("Martin Luther King Jr. Day", mlk_day, "Federal holiday honoring civil rights leader Martin Luther King Jr."))
    
    # Presidents' Day - 3rd Monday in February
    presidents_day = get_nth_weekday(year, 2, 0, 3)
    holidays.append(("Presidents' Day", presidents_day, "Federal holiday honoring all U.S. presidents."))
    
    # Memorial Day - Last Monday in May
    memorial_day = get_last_weekday(year, 5, 0)
    holidays.append(("Memorial Day", memorial_day, "Federal holiday honoring military personnel who died in service."))
    
    # Juneteenth - June 19
    holidays.append(("Juneteenth National Independence Day", date(year, 6, 19), "Federal holiday commemorating the emancipation of enslaved African Americans."))
    
    # Independence Day - July 4
    holidays.append(("Independence Day", date(year, 7, 4), "Federal holiday celebrating the Declaration of Independence."))
    
    # Labor Day - 1st Monday in September
    labor_day = get_nth_weekday(year, 9, 0, 1)
    holidays.append(("Labor Day", labor_day, "Federal holiday celebrating the contributions of workers."))
    
    # Columbus Day - 2nd Monday in October
    columbus_day = get_nth_weekday(year, 10, 0, 2)
    holidays.append(("Columbus Day", columbus_day, "Federal holiday commemorating Christopher Columbus's arrival in the Americas."))
    
    # Veterans Day - November 11
    holidays.append(("Veterans Day", date(year, 11, 11), "Federal holiday honoring military veterans."))
    
    # Thanksgiving Day - 4th Thursday in November
    thanksgiving = get_nth_weekday(year, 11, 3, 4)  # 3 = Thursday
    holidays.append(("Thanksgiving Day", thanksgiving, "Federal holiday for giving thanks and celebrating the harvest."))
    
    # Christmas Day - December 25
    holidays.append(("Christmas Day", date(year, 12, 25), "Federal holiday celebrating the birth of Jesus Christ."))
...
def main():
    ...
    for title, event_date, description in all_holidays:
        cursor.execute(insert_query, (event_date.isoformat(), event_date.year, title, description, title))
