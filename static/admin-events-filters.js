...
(function() {
    ...
    function filterEvents(tabType) {
        ...
        const rows = document.querySelectorAll(getTableSelector(tabType));
        let visibleCount = 0;
    ...
    // filterEvents / clearFilters are bound below via delegation; no window export needed.
    // Bind filter inputs/selects and clear buttons via delegation.
    // Elements carry data-filter-events="<tabType>" or data-clear-filters="<tabType>".
    document.addEventListener('DOMContentLoaded', function() {
        document.addEventListener('input', function(e) {
            const el = e.target.closest('[data-filter-events]');
            if (el) filterEvents(el.dataset.filterEvents);
        });
        ...
        document.addEventListener('click', function(e) {
            const el = e.target.closest('[data-clear-filters]');
            if (el) clearFilters(el.dataset.clearFilters);
        });
    });
})();
