...
(function() {
    ...
    function editEvent(id, date, eventType, name, description, hasPoll) {
        ...
        // Lakes are loaded once on page load by admin-events-lakes.js loadLakes().
        // Just use the cache; ensureLakesLoadedAndEdit tolerates an empty cache.
    ...
    function setupEventListeners() {
        ...
        // Delegated listener for edit-event buttons (data-event-id)
        document.addEventListener('click', function(e) {
            const editBtn = e.target.closest('.js-edit-event');
            if (editBtn) editEvent(editBtn.dataset.eventId);
        });

        // Confirm-delete buttons inside the delete modals
        const confirmCurrentBtn = document.getElementById('confirmDeleteCurrentEventBtn');
        if (confirmCurrentBtn) confirmCurrentBtn.addEventListener('click', confirmDeleteCurrentEvent);
        const confirmPastBtn = document.getElementById('confirmDeletePastEventBtn');
        if (confirmPastBtn) confirmPastBtn.addEventListener('click', confirmDeletePastEvent);
    }
