...
def generate_tournaments(args):
    ...
    for i in xrange(0, args.num):
        ...
        name = '%s %s LAKE %s Tournament #%d -  %d day' % (
            args.club, state, lake, i+1, args.days)
        print('Creating tournament: %s' % name)
        tournament = Tournament.objects.create(
            name='%s - %s- LAKE %s %d day Tournament #%d' % (
                args.club, state, lake, args.days, i+1),
        ...
        for participant in participants:
            results[participant.id] = generate_result(
                tournament, participant, args)
