...
DEFAULT_TOURNAMENT_START: datetime.time = datetime.datetime.time(
    datetime.datetime.strptime("6:00 am", "%I:%M %p")
)
DEFAULT_TOURNAMENT_FINISH: datetime.time = datetime.datetime.time(
    datetime.datetime.strptime("3:00 pm", "%I:%M %p")
)
...
def get_next_event(event_type):
    ...
    event = Events.objects.get(
        type=event_type, month=calendar.month_name[month], year=year
    )
