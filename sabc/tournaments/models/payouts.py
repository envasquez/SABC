...
class PayOutMultipliers(Model):
    ...
    def get_fee_breakdown(self):
        tmnt_pot = sum([self.place_1, self.place_2, self.place_3])  # pyright: ignore[reportCallIssue]
        return "\n".join(
            [
    ...
    def save(self, *args, **kwargs):
        total = sum(  # pyright: ignore[reportCallIssue]
            [
                self.club,
                self.charity,
                self.place_1,
        ...
        self.per_boat_fee = self.entry_fee * 2  # type: ignore
