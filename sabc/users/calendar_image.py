...
def generate_calendar_image(year, events_by_date, width=1200, height=900):
    ...
    draw.text(((width - title_width) // 2, 20), title, fill=colors['black'], font=title_font)
    ...
    month_height = (calendar_height - (spacing * (rows - 1)) - 60) // rows  # 60 for legend
    ...
    for month_num in range(1, 13):
        ...
        # Month border
        draw.rectangle([x, y, x + month_width, y + month_height], 
                      outline=colors['border'], width=1)
        
        # Month header
        header_height = 25
        draw.rectangle([x, y, x + month_width, y + header_height], 
                      fill=colors['header_bg'], outline=colors['border'], width=1)
        
        # Month name
        month_name = month_names[month_num - 1]
        month_bbox = draw.textbbox((0, 0), month_name, font=month_font)
        month_text_width = month_bbox[2] - month_bbox[0]
        draw.text((x + (month_width - month_text_width) // 2, y + 5), 
                 month_name, fill=colors['black'], font=month_font)
        ...
        for i, day_name in enumerate(day_names):
            day_x = x + i * day_width
            draw.rectangle([day_x, day_header_y, day_x + day_width, day_header_y + day_header_height],
                          fill=colors['light_gray'], outline=colors['border'], width=1)
            
            day_bbox = draw.textbbox((0, 0), day_name, font=small_font)
            day_text_width = day_bbox[2] - day_bbox[0]
            draw.text((day_x + (day_width - day_text_width) // 2, day_header_y + 3),
                     day_name, fill=colors['gray'], font=small_font)
        ...
        for week_num, week in enumerate(month_cal):
            for day_num, day in enumerate(week):
                ...
                # Draw day cell
                cell_color = colors.get(bg_color, colors['black']) if bg_color != 'white' else (255, 255, 255)
                draw.rectangle([day_x, day_y, day_x + day_width, day_y + day_height],
                              fill=cell_color, outline=colors['border'], width=1)
                ...
                if day_date == today:
                    ...
                    if bg_color == 'white':
                        draw.rectangle([day_x, day_y, day_x + day_width, day_y + day_height],
                                      fill=(13, 110, 253), outline=colors['border'], width=1)
                
                draw.text((day_x + (day_width - day_text_width) // 2, 
    ...
    # Color box
    draw.rectangle([legend_start_x, legend_y, legend_start_x + 15, legend_y + 15],
                  fill=colors['tournament'], outline=colors['border'], width=1)
    
    # Label
    draw.text((legend_start_x + 20, legend_y), "Tournament Days", fill=colors['black'], font=small_font)
